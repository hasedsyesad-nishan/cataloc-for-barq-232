...
7. **محرك الموشن ديزاين (الأسلوب المعتمد الجديد):** المشاهد تُبنى بأنيميشن CSS حقيقي (نصوص تنزلق بتقسيم كلمات مع تأخيرات متدرجة، cubic-bezier(.16,1,.3,1)، مسح ضوئي sweep، عداد odometer بـ translateY، امتلاء بطارية، طفو المنتج floaty) ثم تُصوَّر كإطارات: Playwright يفتح الصفحة بمقاس 1080×1920 ويثبّت الزمن لكل إطار عبر `document.getAnimations().forEach(a => { a.pause(); a.currentTime = t })` ويلتقط سكرين شوت لكل إطار بمعدل 24fps، ثم تُجمَّع الإطارات بـ ffmpeg (framerate 24 + fade + concat). الهوية البصرية: أسلوب صحفي Editorial — أسود فحمي، ذهبي هادئ #d9a441، حبيبات فيلم grain وvignette، بدون توهجات أو إيموجي.
8. **العلم السوري:** يُمنع استخدام إيموجي 🇸🇾 (يعرض العلم القديم). يُرسم العلم الجديد دائماً كـ SVG: ثلاثة أشرطة أفقية — أخضر ‎#007A3D‎ فوق، أبيض بالوسط، أسود تحت، وثلاث نجوم خماسية حمراء ‎#CE1126‎ في الشريط الأبيض.

---

## قواعد مصداقية إلزامية (لكل المحتوى القادم)
1. **لا تذكر المدى بالأمتار** (مثل ٢٠٠م) — المدى غير ثابت، يعتمد على عدد الجدران وسماكتها. استخدم بدلاً منه: «لاسلكي» أو «يغطي المنزل».
2. **لا تذكر «ترياك BTA40»** — للسخان استخدم: «يتحمل حتى 30 أمبير».
3. **الأرقام المفردة داخل نص عربي** (خاصة الرقم 1): اكتبها لاتينياً وبين قوسين «(1)» حتى لا تُقرأ كحرف ألف. مثال: «ولا (1) أمبير».
